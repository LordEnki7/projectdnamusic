...
*   **Unified Admin Dashboard**: A comprehensive admin interface for managing content, orders, fan engagement, and an AI Command Center.
    *   **Radio Outreach Agent**: A dedicated admin section (`Outreach` tab) that researches college and B-market radio stations via AI, generates personalized pitch emails for each station using OpenAI, and sends them via Resend with a single confirm button. Three sub-tabs: Stations (manage contact list), Campaigns (create and prepare), Results (track sent emails). DB tables: `radio_stations`, `outreach_campaigns`, `outreach_contacts`.
